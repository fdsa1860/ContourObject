...
tic
for si=1:length(seg)
    [seg_tmp,~,~,R] = fast_incremental_hstln_mo(seg{si}',eta_thr);
    R
    seg2{si} = seg_tmp';
    plot(seg{si}(:,1),seg{si}(:,2),'b-*');
    hold on;plot(seg2{si}(:,1),seg2{si}(:,2),'r-*');hold off;
    T = findTranslation(seg2{si},R);
    seg3{si} = bsxfun(@plus,seg2{si},T);
    hold on;plot(seg3{si}(:,1),seg3{si}(:,2),'g-*');hold off;
    35;
end
toc
...
[label,X_center,cntrInd,W] = nCutContour(seg2,3);
% for si = 1:length(seg)
