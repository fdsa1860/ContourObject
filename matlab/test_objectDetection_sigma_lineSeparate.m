...
%% add points
dscA_notLine_all_data = addPointsAll(dscA_notLine_all_data, points_notLine_all);
...
cells_all = cell(1, length(slope_all));
for i = 1:length(slope_all)
    cells_all{i} = genCells([1 1 imgSize_all(i,2) imgSize_all(i,1)], 4, 16);
end

% structured line feature
nBins = 9;
featLine = structureLineFeatAll(slope_all, nBins, points_line_all, cells_all);
...
featNotLine = structureBowFeatHHSigmaAll(dscA_notLine_all_data, centers, alpha, cells_all);
