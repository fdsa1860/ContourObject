% extract contour with fixed longth and sliding window (with smooth)
%%
function [contour_sw,junction_con]=extract_contoursnew(BW,sw)
...
for k = 1:length(contour_clean)
    cur_contour = contour_clean{k};
    for n = 1:length(cur_contour)-sw+1
        contour_sw{count} = cur_contour(n:n+sw-1,:);
        count = count + 1;        
    end
...
% 
 ...
 imshow(0*BW,'border','tight');
 hold on;
 colorz=rand(3,length(contour_sw));
% % 
 for(k=1:length(contour_sw))
      cc=contour_sw{k};
