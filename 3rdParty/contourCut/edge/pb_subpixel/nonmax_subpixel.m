function [im,x,y] = nonmax_subpixel(im,theta)
% function [im,x,y] = nonmax_subpixel(im,theta)
%
...
% Interpolate pixel locations
x = -ones(size(im));
y = -ones(size(im));

is_interior = ix<w & ix>1 & iy<h & iy>1;
% case 1,5
idx = find( mask15 & is_interior );
idxA = idx + h;
idxB = idx + h + 1;
idxC = idx - h;
idxD = idx - h - 1;
d = tan(theta(idx));
imI1 = im(idxA).*(1-d) + im(idxB).*d;
imI2 = im(idxC).*(1-d) + im(idxD).*d;
max_x = max_parab([imI2, im(idx), imI1]);
% max_x(find(im(idx)<imI1 | im(idx)<imI2)) = 0;
x(idx) = floor((idx-1)/h) + 1 + max_x;
y(idx) = mod(idx-1, h) + 1 + max_x.*d;

% case 2,6
idx = find( mask26 & is_interior );
idxA = idx + 1;
idxB = idx + h + 1;
idxC = idx - 1;
idxD = idx - h - 1;
...
max_x = max_parab([imI2, im(idx), imI1]);
% max_x(find(im(idx)<imI1 | im(idx)<imI2)) = 0;
x(idx) = floor((idx-1)/h) + 1 + max_x.*d;
y(idx) = mod(idx-1, h) + 1 + max_x;

% case 3,7
idx = find( mask37 & is_interior );
idxA = idx + 1;
idxB = idx - h + 1;
idxC = idx - 1;
idxD = idx + h - 1;
